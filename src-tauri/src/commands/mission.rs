...
use crate::mission::contextpack_staleness::{check_contextpack_staleness, ContextPackStalenessStatus};
...
// ── M2: Layer1 / ContextPack commands ──────────────────────────
    ...
    match input.kind {
        Layer1ArtifactKind::ChapterCard => {
            ...
            if doc.scope_ref.trim().is_empty() {
                return Err(AppError::invalid_argument("chapter_card.scope_ref cannot be empty"));
            }
        ...
        Layer1ArtifactKind::RecentFacts => {
            ...
            if doc.scope_ref.trim().is_empty() {
                return Err(AppError::invalid_argument("recent_facts.scope_ref cannot be empty"));
            }
        ...
        Layer1ArtifactKind::ActiveCast => {
            ...
            if doc.scope_ref.trim().is_empty() {
                return Err(AppError::invalid_argument("active_cast.scope_ref cannot be empty"));
            }
...
pub async fn mission_contextpack_rebuild_if_stale(
    ...
    let token_budget = serde_json::to_string(&rebuilt.token_budget)
        ...
        .to_string();
    let emitter = MissionEventEmitter::new(app_handle, input.mission_id.clone());
    if let Err(e) = emitter.contextpack_built(&rebuilt.scope_ref, &token_budget, rebuilt.generated_at) {
        tracing::warn!(target: "mission", error = %e, "failed to emit MISSION_CONTEXTPACK_BUILT");
    }
